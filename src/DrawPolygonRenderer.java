...
public class DrawPolygonRenderer extends GLCanvas implements GLEventListener, MouseListener, KeyListener, MouseMotionListener, MouseWheelListener {
    ...
    private float lastMiddlePivot[] = {0f, 0f};
...
//            gl.glEnable( GL2.GL_NORMALIZE );
    ...
    private void select2DMode(GL2 gl, Polygon renderPolygon) {
        // 绘制CAT
        if (renderMode == EDIT_MODE) {
            if (editMode == SHOW_OUTSIDELINE_MODE) {
                drawOutLine(gl, renderPolygon);
            }
            if (editMode == SHOW_CATLINE_MODE) {
...
//                gl.glDisable(GL_LINE_STIPPLE);
            ...
            if (editMode == SHOW_FANLINE_MODE || editMode == SHOW_AXIS_MODE || editMode == SHOW_FANRIGION_MODE) {
                ...
                if (editMode == SHOW_AXIS_MODE || editMode == SHOW_FANRIGION_MODE) {
                    ...
                    if (editMode == SHOW_FANRIGION_MODE) {
...
//                gl.glDisable(GL_LINE_STIPPLE);
    ...
    private void drawOutLine(GL2 gl, Polygon renderPolygon) {
        if (renderMode == EDIT_MODE && editMode == SHOW_MODEL_MODE) {
            gl.glColor3f(1.0f, 0.0f, 0.0f);
            float startZ = 0f;
            float endZ = 0f;
            for (Model m : modelList) {
                float tempZ = (float) m.getBiggestZ();
                if (tempZ > startZ || tempZ > endZ) {
                    startZ = tempZ;
                    endZ = tempZ;
                }
            }
            for (int i = 0; i < modelList.get(thisModel).getEdgeCoords().length; i++) {
                int nextIndex = 0;
                if (i != modelList.get(thisModel).getEdgeCoords().length - 1) {
                    nextIndex = i + 1;
                }
                gl.glHint(GL_LINE_SMOOTH_HINT, GL_FASTEST);
                gl.glLineWidth(8.0f);
                ...
                gl.glVertex3f((float) modelList.get(thisModel).getVertexPoint().get(modelList.get(thisModel).getEdgeCoords()[i]).getX(), (float) modelList.get(thisModel).getVertexPoint().get(modelList.get(thisModel).getEdgeCoords()[i]).getY(), startZ);
                gl.glVertex3f((float) modelList.get(thisModel).getVertexPoint().get(modelList.get(thisModel).getEdgeCoords()[nextIndex]).getX(), (float) modelList.get(thisModel).getVertexPoint().get(modelList.get(thisModel).getEdgeCoords()[nextIndex]).getY(), endZ);
            ...
            for (int i = 0; i < renderPolygon.getPolygonLine().size(); i++) {
                gl.glColor3f(0.0f, 0.0f, 0.0f);
//            if (renderMode == CUT_MODE || renderMode == EDIT_MODE) {
//                gl.glColor3f(1.0f, 0.0f, 0.0f);
//            }

                float startZ = (float) renderPolygon.getPolygonLine().get(i).getStart().getZ();
                float endZ = (float) renderPolygon.getPolygonLine().get(i).getEnd().getZ();
                gl.glEnable(GL_LINE_SMOOTH);
                gl.glHint(GL_LINE_SMOOTH_HINT, GL_FASTEST);
                gl.glLineWidth(8.0f);
                gl.glBegin(GL2.GL_LINES);

                gl.glVertex3f((float) renderPolygon.getPolygonLine().get(i).getStart().getX(), (float) renderPolygon.getPolygonLine().get(i).getStart().getY(), startZ);
                gl.glVertex3f((float) renderPolygon.getPolygonLine().get(i).getEnd().getX(), (float) renderPolygon.getPolygonLine().get(i).getEnd().getY(), endZ);
...
//                System.out.println(thisModel);
    ...
    private void countModelNum(int x, int y, GL2 gl) {
        thisModel = -1;
        ...
        this.glu.gluUnProject((double) x, (double) realY, 0, modelViewMatrix, 0, projectionMatrix, 0, viewport, 0, thisVertex, 0);
        Point thisPoint = new Point(thisVertex[0], thisVertex[1], 0);

        gl.glBegin(GL2.GL_POINTS);
        gl.glVertex3f((float) thisPoint.getX(), (float) thisPoint.getY(), (float) 1000000);
        gl.glEnd();

        for (int i = 0; i < modelList.size(); i++) {
            if (modelList.get(i).getBasedPolygon().inPolygon(thisPoint)) {
                thisModel = i;
                return;
            }
        }
...
//            modelCount++;
    ...
    //备份
    public void packup() {
//        System.out.println("Prepare to Packup, Old List Count: " + oldModelList.size() + ", Current List Count: " + modelList.size());
        oldModelList.clear();
...
//        System.out.println("Finish to Packup, Old List Count: " + oldModelList.size() + ", Current List Count: " + modelList.size());
    }

    //Undo，返回到上一个可渲染的模型中
    public void undo() {
//        System.out.println("Prepare to undo, Old List Count: " + oldModelList.size() + ", Current List Count: " + modelList.size());
        modelList.clear();
